...
$$
E_p[f(x)] = \sum f(x)p(x)dx
$$

条件数学期望:

给定一个条件概率密度函数 p(x|y), 再给定一个函数 f(x)，定义 数学期望（Conditional Expectation）为：
$$
...
**找到使目前似然函数最大的那个观测或者由于对数变换是单调变化，找到负的对数似然函数最小的那个。**
...
$$
换句话说，因为E_\theta^*[log p_\theta^*(X,y)|X]跟 θ 没有关系，所以可以忽略。
$$
...
(d) 上标表示观测d；\\
π_{z^{(d)}_1}为初始分布；\\
A_{z^{(d)}_{t−1}{z^{(d)}_{t}}}为转移概率；\\
B_{{Z_{t}^{(d)}}}(X^{(d)}_{t})为发射概率；
$$
对上式取 log 之后:
$$
